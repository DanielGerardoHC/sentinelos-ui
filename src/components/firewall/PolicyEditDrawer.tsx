...
import { ShieldCheck, Save, Plus, X } from "lucide-react";
...
export function PolicyEditDrawer({ isOpen, onClose, policyData, onSuccess, onError }: PolicyEditDrawerProps) {
    ...
    const availableServices = services.filter(svc => !formServices.includes(svc.name));
                ...
                >
                    ...
                    <div className="p-6 space-y-6 flex-1 overflow-y-auto relative">
                        ...
                        <div className="grid grid-cols-2 gap-6 p-4 rounded-lg border border-zinc-800/50 bg-zinc-900/20">
                            <div className="space-y-3">
                                ...
                                <select value={formSrcZone} onChange={(e) => setFormSrcZone(e.target.value)} className="w-full h-11 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 font-mono focus:ring-1 focus:ring-emerald-500/50 outline-none">
                                    <option value="">{t('policies.any', 'ANY')}</option>
                        ...
                        <div className="space-y-3 p-4 rounded-lg border border-zinc-800/50 bg-zinc-900/20">
                            <Label className="text-zinc-500 font-mono text-xs uppercase flex justify-between items-center">
                                {t('policy_drawer.services', 'Services')}
                            ...
                            <div className="space-y-2">
                                {formServices.length === 0 && (
                                    <div className="text-emerald-500/70 text-xs font-mono p-3 border border-emerald-500/20 border-dashed rounded bg-emerald-500/5 text-center font-bold tracking-widest">
                                        {t('policies.any', 'ANY (All Traffic)')}
                                    </div>
                                )}
                                {formServices.map(svcName => (
                                    <div key={svcName} className="flex justify-between items-center p-2 rounded-md border border-zinc-800 bg-zinc-950">
                                        <span className="font-mono text-sm text-emerald-400 font-bold">{svcName}</span>
                                        <Button
                                            type="button"
                                            variant="outline"
                                            size="icon"
                                            onClick={() => setFormServices(prev => prev.filter(s => s !== svcName))}
                                            className="h-7 w-7 bg-transparent border-transparent text-zinc-400 hover:text-red-400"
                                        >
                                            <X className="w-3 h-3" />
                                        </Button>
                                    </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <select
                                    ...
                                    className="flex-1 h-11 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 font-mono focus:ring-1 focus:ring-emerald-500/50 outline-none"
                                >
                                    <option value="" disabled>{t('policy_drawer.add_service', 'Add service...')}</option>
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        if (selectedServiceToAdd) {
                                            setFormServices([...formServices, selectedServiceToAdd]);
                                            setSelectedServiceToAdd('');
                                        }
                                    }}
                                    disabled={!selectedServiceToAdd}
                                    className="h-11 px-4 bg-zinc-800 hover:bg-emerald-600 text-white font-mono transition-colors"
                                >
